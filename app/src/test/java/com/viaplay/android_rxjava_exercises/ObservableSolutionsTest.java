...
public class ObservableSolutionsTest {
    ...
    public void exerciseFlatMap() {
        ...
        Map<Integer, Integer> map = getImpl().exerciseFlatMap(movies).toMap(new Func1<Integer, Integer>() {
            @Override
    ...
    public void exerciseMovie() {
        ...
        Map<Integer, JSON> map = getImpl().exerciseMovie(movies).toMap(new Func1<JSON, Integer>() {
            @Override
            public Integer call(JSON json) {
    ...
    public void handleError() {
        TestSubscriber<String> ts = new TestSubscriber<>();
        getImpl().handleError(Observable.<String>error(new RuntimeException("failure"))).subscribe(ts);
        ts.awaitTerminalEvent();
        ts.assertNoErrors();
        ...
        Observable<String> o = Observable.create(new Observable.OnSubscribe<String>() {
            ...
            public void call(Subscriber<? super String> subscriber) {
                if (c.incrementAndGet() <= 1) {
                    subscriber.onError(new RuntimeException("fail"));
                } else {
